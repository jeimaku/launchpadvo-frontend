...
export default function LPCVirtualOffice() {
  ...
  const [formData, setFormData] = useState({
    ...
    rate_per_month: '',
    payment_info: '',
    payment_terms: '', // NEW FIELD ADDED HERE
    remarks: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/virtual-offices', {
        ...
        body: JSON.stringify({ ...formData, branch: 'LPC' }) 
      });
                ...
                {clients.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-12 text-center text-slate-500">
                      No LPC clients found. Click "Add New Client" to populate the list!
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-4xl rounded-2xl bg-white p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="mb-6 flex items-center justify-between border-b border-slate-100 pb-4">
              <h3 className="text-xl font-bold text-slate-800">Register LPC Client</h3>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                {/* THE NEW PAYMENT TERMS DROPDOWN */}
                <div>
